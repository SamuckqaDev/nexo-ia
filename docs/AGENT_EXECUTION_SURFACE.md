...
walkthrough is text + citations + saved knowledge + bounded `workspace_*` and `mcp_*` evidence,
including a single-file Workspace write only for a direct authorized Agent request.
